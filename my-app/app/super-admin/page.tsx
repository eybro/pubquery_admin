...
function timeAgoColor(last?: Date) {
  ...
  if (diffMin < 60)
    return { label: "OK", tone: "success", text: `${diffMin} min` };
  if (diffMin < 180)
    return {
      label: "Stale",
      tone: "warning",
      text: `${Math.floor(diffMin / 60)} h`,
    };
...
export default function Page() {
  ...
  // UI toggle: hide admin/demo (default ON)
  ...
  const fetchCounters = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/admin/monitor/counters`,
        { credentials: "include" },
      );
  ...
  const visibleLogs = useMemo(() => {
    if (!hideSystemUsers) return logs;
    return logs.filter((item) => {
      const u = (item.username ?? "").toLowerCase();
      return u !== "admin" && u !== "demo";
    });
  }, [logs, hideSystemUsers]);
                      ...
                      <TableBody>
                        {counters?.map((r) => {
                          const last = r.last_activity_at
                            ...
                            : undefined;
                          const hasLine =
                            !!r.line_length && r.line_length !== "no_line";
                          return (
                            <TableRow key={r.venue_id}>
                              ...
                              <TableCell>
                                <Badge
                                  className={
                                    r.visible ? "bg-green-600 text-white" : ""
                                  }
                              ...
                              <TableCell>
                                <Badge
                                  ...
                                  variant={
                                    r.ratio_visible ? undefined : "secondary"
                                  }
                                >
                              ...
                              <TableCell>
                                <Badge
                                  className={
                                    hasLine ? "bg-green-600 text-white" : ""
                                  }
                                ...
                                <TooltipProvider>
                                  <Tooltip>
                                    ...
                                    <TooltipContent>
                                      <p>
                                        {last
                                          ? `Last activity: ${last.toLocaleString()}`
                                          : "No socket activity recorded"}
                                      </p>
                                    </TooltipContent>
                                  </Tooltip>
                                </TooltipProvider>
              ...
              <CardContent className="p-4 sm:p-6">
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Label
                      htmlFor="hide-system-users"
                      className="text-xs sm:text-sm"
                    >
                      Hide admin/demo
                    </Label>
                    <Switch
                      id="hide-system-users"
                      checked={hideSystemUsers}
                      onCheckedChange={setHideSystemUsers}
                    />
                  </div>
                </div>
                  ...
                  <ScrollArea className="h-[520px] pr-2">
                    ...
                    {!logsLoading && visibleLogs.length === 0 && (
                      <div className="mt-2 px-1 text-xs text-muted-foreground">
                        No events (possibly filtered out).
                      </div>
                    )}
                  </ScrollArea>
